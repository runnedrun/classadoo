...
'<h2>(a realtime html + css editor)</h2>'].join('\n');
  ...
  if (Scratchpad.document_id != "classadoo-instructor") {
    var syncPreviewRef = new Firebase('https://classadoo-scratch.firebaseIO.com/classadoo-instructor');
    var syncPreview = new ScratchpadSyncPreview(editor, syncPreviewRef)

    syncPreviewRef.on('value', function(dataSnapshot) {
      console.log("getinting vl", dataSnapshot.child('code').val());
